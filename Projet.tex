...
$\cdot$ se traduit en l'égalité des deux arbres syntaxiques
\begin{equation}
    \begin{split}
...
deux opérateurs pour en construire un plus gros. Au même titre que les 
...
syntaxiques qui ne sont plus des arbres mais plutôt des graphes orienté, 
où les n\oe uds internes disposent potentiellement de plusieurs entrées 
et de plusieurs sorties.

À titre d'exemple, la relation entre le produit $\cdot$ et le coproduit
$\Delta$ d'une algèbre de Hopf s'écrit 
$\Delta(x \cdot y) = \Delta(x) \cdot \Delta(y)$ et celle-ci se traduit 
en l'égalité entre les deux arbres syntaxiques généralisés
\begin{equation}
    \begin{split}
...
historiquement avant les opérades), généralisent les opérades et 
...
\begin{enumerate}
    ...
    sur ces opérateurs en fonction du nombre d'opérateurs utilisés~;
    ...
    zone. À titre d'exemple, un prographe permet de modéliser de manière
    évidente des circuits électroniques (voir 
    figure~\ref{fig:exemple_prographe_circuit})
    \begin{figure}[ht]
        \centering
    ...
    algébrique des circuit peut apporter~;
    ...
    tailles) et leurs compositions. En un certain sens, ce PRO contient 
    ...
    et de savoir ce que cette vision peut apporter nous intéresse~;
    ...
    Nous espérons dans ce projet une interaction riche et nouvelle entre 
    la théorie des cartes et la théorie des PROs. Il semble d'une part 
    pouvoir être possible d'utiliser certains résultats énumératifs des 
    cartes pour le dénombrement des prographes et certains résultats 
    algébriques des PROs en théorie des cartes.
\end{enumerate}

Nous espérons, à l'issue de la réalisation de ce projet, soumettre un 
article à un journal de combinatoire algébrique ainsi qu'un article à 
une conférence du même domaine.
...
Nous présentons ici quelques résultats expérimentaux.
